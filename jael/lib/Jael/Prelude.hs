...
-- haskus-utils
--import Haskus.Utils.Variant as X
--import Haskus.Utils.Variant
---- Re-exports Data.Functor.Foldable :/
import Haskus.Utils.VariantF as X hiding
  (fold, gfold, grefold, gunfold, refold, unfold)
...
import Control.Monad.RWS      as X hiding (First, Last, getFirst, getLast, (<>))
import Control.Monad.State    as X
import Control.Monad.Writer   as X hiding (First, Last, getFirst, getLast, (<>))
...
$(mkVariantInstances ''Data.TreeDiff.ToExpr)
$(mkVariantInstances ''Foldable)

instance Eq1 (VariantF '[]) where
   liftEq _ _ _ = error "Empty variant"

instance (Eq1 f, Eq1 (VariantF fs)) => Eq1 (VariantF (f ': fs)) where
  liftEq f as bs = case (popVariantFHead as, popVariantFHead bs) of
    (Right a', Right b') -> liftEq f a' b'
    (Left as', Left bs') -> liftEq f as' bs'
    _                    -> False

$(mkVariantInstances ''Show1)
...
digitToInt c = toInteger <$>
  if | (fromIntegral dec  :: P.Word) <= 9 -> Just   dec
